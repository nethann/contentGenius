...
import { Upload, Play, Scissors, Download, Clock, Zap, FileVideo, FileAudio, AlertCircle, Mic } from 'lucide-react';
...
const ContentScalar = () => {
  ...
  const generateCaptionsFromVideoAudio = async (videoFile, startTime, endTime) => {
    if (!speechSupported) {
  ...
  const handleFileUpload = (event) => {
    ...
    if (uploadedFile) {
      const validTypes = ['video/mp4', 'audio/mp3', 'audio/mpeg', 'video/quicktime', 'audio/wav'];
  ...
  const generateVideoClip = async (moment) => {
    ...
    try {
      ...
      if (file.type.startsWith('video') && speechSupported) {
        try {
          videoCaptions = await generateCaptionsFromVideoAudio(file, startTime, endTime);
      ...
      let subtitles = [];
      try {
        const subtitleResponse = await fetch("https://api.anthropic.com/v1/messages", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "x-api-key": import.meta.env.VITE_ANTHROPIC_API_KEY || "your-api-key-here",
...
Use natural speech timing and the ${moment.category} category flow. DO NOT include any text outside the JSON.`
        ...
        if (subtitleResponse.ok) {
          ...
          let responseText = subtitleData.content[0].text;
      ...
      if (videoCaptions.length > 0) {
        ...
        if (subtitles.length > 0 && subtitles[subtitles.length - 1].end < duration - 2) {
        ...
        if (subtitles.length === 0) {
          subtitles = [
            { start: 0, end: duration * 0.5, text: moment.transcript.slice(0, 60) || "Check out this moment!" },
            { start: duration * 0.5, end: duration, text: moment.transcript.slice(60) || "Don't miss this!" }
          ];
        }
      }
      ...
      console.log(`✅ Successfully generated clip for "${moment.title}" with ${subtitles.length} subtitle segments`);
  ...
  const showVideoClipModal = (moment) => {
    ...
    `;
    realtimeCaptionsOverlay.textContent = 'Real-time captions will appear here...';
    ...
    timeDisplay.style.cssText = 'color: white; font-size: 14px; min-width: 80px;';
    timeDisplay.textContent = '0:00 / 0:00';
    
    // Close button
    // Speech recognition toggle button
    const speechToggleBtn = document.createElement('button');
    speechToggleBtn.innerHTML = speechSupported ? '🎤' : '🚫';
    speechToggleBtn.style.cssText = `
      position: absolute; top: 20px; left: 20px;
      background: ${speechSupported ? 'rgba(0,255,136,0.3)' : 'rgba(255,0,0,0.3)'};
      border: 1px solid ${speechSupported ? 'rgba(0,255,136,0.5)' : 'rgba(255,0,0,0.5)'};
      color: white; font-size: 20px; width: 45px; height: 45px;
      border-radius: 50%; cursor: pointer; backdrop-filter: blur(10px);
      transition: all 0.3s ease;
    `;
    speechToggleBtn.title = speechSupported ? 'Click to enable real-time speech captions (Ctrl+C)' : 'Speech recognition not supported in this browser';
    ...
    const updateRealtimeCaptions = () => {
      if (isRealtimeCaptionsActive && isListening) {
        realtimeCaptionsOverlay.textContent = realtimeCaptions || 'Listening...';
        realtimeCaptionsOverlay.style.opacity = '1';
      } else if (isRealtimeCaptionsActive) {
        realtimeCaptionsOverlay.textContent = 'Real-time captions ready - Press 🎤 to activate';
    ...
    const updateSubtitles = () => {
      ...
      const currentRelativeTime = video.currentTime - moment.startTime;
      ...
      const currentSubtitle = moment.subtitles?.find(sub => 
      ...
      if (currentSubtitle) {
        ...
        subtitleOverlay.style.opacity = '1';
        subtitleOverlay.style.background = 'linear-gradient(to top, rgba(0,0,0,0.9), rgba(0,0,0,0.7))';
      } else {
        ...
        const contextWords = transcriptWords.slice(Math.max(0, wordIndex - 4), wordIndex + 4).join(' ');
        
        subtitleOverlay.textContent = contextWords || moment.title;
        subtitleOverlay.style.opacity = '0.7';
        subtitleOverlay.style.background = 'linear-gradient(to top, rgba(0,0,0,0.7), rgba(0,0,0,0.5))';
      }
    };
    
    const updateProgress = () => {
      const currentRelativeTime = video.currentTime - moment.startTime;
      const progress = Math.min(Math.max(currentRelativeTime / segmentDuration, 0), 1) * 100;
    ...
    const togglePlay = () => {
      if (isPlaying) {
        ...
        if (video.currentTime < moment.startTime || video.currentTime >= moment.endTime) {
          video.currentTime = moment.startTime;
        }
        video.play();
        playBtn.innerHTML = '⏸️';
    ...
    video.addEventListener('timeupdate', () => {
      // Stop at end time
      if (video.currentTime >= moment.endTime) {
        video.pause();
        playBtn.innerHTML = '▶️';
        isPlaying = false;
        video.currentTime = moment.startTime; // Reset for replay
    ...
    const startSubtitleUpdates = () => {
      ...
      subtitleUpdateInterval = setInterval(() => {
        if (isPlaying && video.currentTime >= moment.startTime && video.currentTime <= moment.endTime) {
          updateSubtitles();
        }
    ...
    video.currentTime = moment.startTime;
    setTimeout(() => {
      video.currentTime = moment.startTime; // Double-ensure we're at the right position
      updateSubtitles();
    }, 200);
    ...
    progressBar.onclick = (e) => {
      ...
      const newTime = moment.startTime + (clickPercent * segmentDuration);
      video.currentTime = Math.min(Math.max(newTime, moment.startTime), moment.endTime - 0.1);
    };
  ...
  const previewClip = (moment) => {
    if (!moment.clipGenerated) {
      alert('Generate clip first to preview');
      return;
    }
  ...
  const generateFallbackAnalysis = (fileInfo) => {
    ...
    const isAudio = fileInfo.type.includes('audio') || fileInfo.type.includes('mp3');
    const durationMinutes = Math.floor(fileInfo.duration / 60);
    
    const moments = [];
    
    // Generate 3-5 moments based on file duration
    const numMoments = Math.min(5, Math.max(3, Math.floor(durationMinutes / 2)));
    
    const categories = ['funny', 'educational', 'emotional', 'engaging'];
    const titleTemplates = {
      funny: ['The Brutal Truth About Comedy', 'Why 99% Get This Wrong', 'The Secret Nobody Talks About', 'This Changed Everything'],
      educational: ['The 80/20 Rule Explained', 'What School Never Taught You', 'The Hard Truth About Success', 'Why Smart People Fail'],
      emotional: ['The Price of Following Dreams', 'What Nobody Tells You', 'The Moment Everything Changed', 'Why Most People Give Up'],
      engaging: ['This Will Blow Your Mind', 'The Uncomfortable Truth', 'What Everyone Gets Wrong', 'The Reality Check You Need']
    };
    ...
    for (let i = 0; i < numMoments; i++) {
      ...
      const selectedTranscription = transcriptionTemplates[category][Math.floor(Math.random() * transcriptionTemplates[category].length)];
      
      moments.push({
        title: titleTemplates[category][Math.floor(Math.random() * titleTemplates[category].length)],
        ...
        suggestedCaption: `"${selectedTranscription.slice(0, 50)}..." Don't miss this! 🔥 #viral #${category}`
      });
    }
      ...
      try {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "user",
              content: `I'm uploading a ${fileInfo.type} file named "${fileInfo.name}" that is ${(fileInfo.size / (1024 * 1024)).toFixed(1)}MB and approximately ${Math.floor(fileInfo.duration / 60)} minutes long.

Based on the file "${fileInfo.name}" and its content type, generate realistic viral moments with ACTUAL TRANSCRIPTIONS of what would be spoken during those specific 30-second segments. Think about what a person would realistically be saying during those exact timestamps in this video.
...
DO NOT include any text outside the JSON. Make realistic timestamps based on the ${Math.floor(fileInfo.duration / 60)} minute duration.`
            }
          ]
        })
      });
        ...
        const processedMoments = fallbackAnalysis.moments?.map((moment, index) => ({
          id: index + 1,
      ...
      if (!analysisResponse.ok) {
        ...
        if (analysisResponse.status === 401 || analysisResponse.status === 400) {
          console.warn('API key missing or invalid. Using fallback demo analysis.');
          const fallbackAnalysis = generateFallbackAnalysis(fileInfo);
          ...
          if (processedMoments.length === 0) {
            setError("No viral moments detected. Try a different file or longer content.");
          }
          return;
        }
      ...
      const processedMoments = aiAnalysis.moments?.map((moment, index) => ({
        id: index + 1,
        title: moment.title || `Moment ${index + 1}`,
        startTime: moment.startTime || "0:00",
        endTime: moment.endTime || "0:30",
        ...
        <div className="max-w-4xl mx-auto">
          {!file ? (
            ...
            >
              ...
              <p className="text-gray-400 mb-4">Drop your MP4 video, MP3 audio, or WAV file here</p>
            ...
            <div className="bg-gray-800 rounded-lg p-6">
                      ...
                      <div className="flex-1">
                        ...
                        {moment.transcript && (
                          <div className="bg-gray-800 p-4 rounded mb-3">
                            <p className="text-yellow-200 text-base italic leading-relaxed whitespace-pre-wrap">"{moment.transcript}"</p>
                          </div>
                        )}
                        
                        {moment.suggestedCaption && (
                          <p className="text-blue-200 text-sm mb-3">
                            <strong>Suggested Caption:</strong> {moment.suggestedCaption}
                          </p>
                        )}
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4 text-sm text-gray-400">
                            ...
                            <span className="bg-gray-600 px-2 py-1 rounded">{moment.duration}</span>
                            {moment.clipGenerated && (
                              <span className="bg-green-600 px-2 py-1 rounded text-xs">✓ Clip Ready</span>
                            )}
                          </div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {clipProgress[moment.id] !== undefined ? (
                              <div className="flex items-center gap-2">
                                <div className="w-20 bg-gray-600 rounded-full h-2">
                                  <div 
                                    ...
                                    style={{ width: `${clipProgress[moment.id]}%` }}
                                  ></div>
                                </div>
                                <span className="text-xs text-gray-400">{Math.round(clipProgress[moment.id])}%</span>
                              </div>
                            ) : (
                              <button
                              ...
                              >
                                ...
                                {moment.clipGenerated ? 'Download' : 'Generate Clip'}
                              </button>
                            )}
                          </div>
                        </div>
              ...
              <div className="mt-6 p-4 bg-green-900/30 rounded-lg">
                <p className="text-green-200 text-sm">
                  <strong className="text-white">🎬 Video Clip Generation!</strong> Click "Generate Clip" to create actual video segments with embedded subtitles. 
                  Each clip will include AI-generated captions perfectly timed to the dialogue. When you preview, you'll see live yellow captions transcribing the actual video audio!
                </p>
              </div>
              ...
              {speechSupported && (
                <div className="mt-4 p-4 bg-blue-900/30 rounded-lg">
                  <p className="text-blue-200 text-sm">
                    <strong className="text-white flex items-center gap-2"><Mic className="w-4 h-4" /> Real-time Captions!</strong> 
                    When previewing clips, click the 🎤 button to enable live speech-to-text captions that transcribe 
                    what the speaker is saying in real-time. Use Ctrl+C to toggle captions on/off during playback.
                  </p>
                </div>
              )}
            </div>
